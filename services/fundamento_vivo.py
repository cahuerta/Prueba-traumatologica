...
def buscar_fundamento(region: str, pregunta: str, opciones: List[str], correcta: int) -> dict:
    ...
    prompt = (
        ...
        + contexto + "\n\n"
        "Antes de redactar, define con precision cual es el cuadro clinico que describe la "
        "pregunta (a partir de los signos, sintomas y contexto del enunciado y las opciones). "
        "Tu fundamentacion debe explicar exclusivamente ese cuadro clinico. No incorpores "
        "diagnosticos, mecanismos fisiopatologicos, hallazgos ni terminologia de una condicion "
        "distinta -aunque sea de la misma region anatomica, aunque parezca relacionada, o aunque "
        "la hayas visto mencionada en el material de referencia. Si tienes duda sobre si un "
        "concepto pertenece al cuadro de la pregunta, descartalo.\n\n"
        "Redacta un resumen CORTO (maximo 4-5 lineas), fundamentado, que explique por que "
        "esa es la respuesta correcta, basandote en el material de referencia entregado. "
        "No copies el texto completo del material, sintetiza el argumento clinico clave."
    )
